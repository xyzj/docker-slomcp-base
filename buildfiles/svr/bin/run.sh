...
echo "Starting mysql"
# service mysql start
start-stop-daemon --start --name mysqld --background --exec /usr/sbin/mysqld -- -name luwak --data-dir /root/etcd/luwak.etcd --cert-file=/opt/ca/server.pem --key-file=/opt/ca/server-key.pem --advertise-client-urls=https://0.0.0.0:2378,http://0.0.0.0:2379 --listen-client-urls=https://0.0.0.0:2378,http://0.0.0.0:2379
...
echo "Starting etcd"
start-stop-daemon --start --name etcd --background -d /root/etcd --exec /root/etcd/etcd -- -name luwak --data-dir /root/etcd/luwak.etcd --cert-file=/opt/ca/server.pem --key-file=/opt/ca/server-key.pem --advertise-client-urls=https://0.0.0.0:2378,http://0.0.0.0:2379 --listen-client-urls=https://0.0.0.0:2378,http://0.0.0.0:2379
sleep 2
