...
echo "Starting etcd"
start-stop-daemon --start -p /run/etcd.pid --background -d /opt/etcd --exec /opt/etcd/etcd -- -name luwak --data-dir /opt/etcd/luwak.etcd --cert-file=/opt/ca/server.pem --key-file=/opt/ca/server-key.pem --advertise-client-urls=https://0.0.0.0:2378,http://0.0.0.0:2379 --listen-client-urls=https://0.0.0.0:2378,http://0.0.0.0:2379
sleep 2

# sslrenew start
start-stop-daemon --start -p /run/sslrenew.pid --background -d /opt/bin --exec /opt/bin/sslrenew
sleep 2
